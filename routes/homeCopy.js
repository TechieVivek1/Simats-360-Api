...
async function calculateAttendanceC(punchResultData, holidayResultdata, shiftTotalHours, shiftStartTime,shiftEndtime, buffResultsData){
    ...
    punch.forEach(punch=>{
        ...
        if (isHolidayOrSundayC(punchData,holidayResultdata)) {
...
const homeInfo = async (req, res) => {
    ...
    try {
        ...
        const {presentDaysC,absentDaysC,attendancePercentageC,adjustedBuffC} = calculateAttendanceC(
